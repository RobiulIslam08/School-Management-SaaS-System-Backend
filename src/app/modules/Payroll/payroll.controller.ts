...
import { createPayroll, listPayroll, markPaid, payrollPayMessage } from "./payroll.service";
...
export const payrollController = {
  ...
  async pay(req: Request, res: Response): Promise<void> {
    const { item, sms } = await markPaid(routeParam(req.params.id));
    ok(res, item, payrollPayMessage(sms));
  },
};
